...
% Shifted displaced coordinates (output of MLP)
%\coordinate (v1') at (1.4,0.1);
%\coordinate (v2') at (2.4,0.3);
%\coordinate (v3') at (2.2,1.1);
%\coordinate (v4') at (1.4,1.1);
%\coordinate (v5') at (3.0,0.7);
%\coordinate (v6') at (2.7,1.4);
%\coordinate (v7') at (3.6,0.4);
%\coordinate (v8') at (3.8,1.3);

% Draw displaced points with specific label placements
%\foreach \i/\pos in {1/below, 2/below, 3/above, 4/above, 5/below, 6/above, 7/below, 8/above} {
%  \fill[gray] (v\i') circle (1pt);
%  \node[\pos=2pt] at (v\i') {$\bm{v_\i'}$};
%}

% Triangulated mesh connections (manual, like in example)
%\draw[gray, dashed] (v1') -- (v2') -- (v3') -- cycle;
%\draw[gray, dashed] (v1') -- (v3') -- (v4') -- cycle;

%\draw[gray, dashed] (v2') -- (v5') -- (v3') -- cycle;
%\draw[gray, dashed] (v3') -- (v5') -- (v6') -- cycle;

%\draw[gray, dashed] (v5') -- (v7') -- (v6') -- cycle;
%\draw[gray, dashed] (v6') -- (v7') -- (v8') -- cycle;
% MLP block (left)
%\node[draw, minimum width=1cm, minimum height=1.8cm, align=center] (mlp) at (0,0.9) {MLP};

% Straight arrow from MLP to right (input to mesh)
%\draw[->, thick] (mlp.east) -- ++(1.0, 0);

%\end{tikzpicture}
%\end{adjustbox}
%\caption{Illustration of neural displacements. The MLP transforms input features into displaced mesh points $\bm{v_i'}$. The dashed gray lines indicate the local mesh structure.}
%\label{fig:neural_displacement}
%\end{figure}
